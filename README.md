...
このツールを使用するためには、以下のPythonライブラリをインストールする必要があります。コマンドプロンプトで以下のコマンドを実行してください：

- **win32gui**: Windowsのウィンドウ操作を行うために使用します。
  - [win32gui](https://pypi.org/project/win32gui/)
- **time**: スクリプト内で待機時間を設定するために使用します。
  - [time](https://pypi.org/project/TIME-python/)
- **PyAutoGUI**: キーボードやマウスの自動操作を行うために使用します。
  - [PyAutoGUI](https://pypi.org/project/PyAutoGUI/)
- **os-sys**: ファイル操作やシステムの操作を行うために使用します。
  - [os-sys](https://pypi.org/project/os-sys/)
- **DateTime**: 日付や時間の管理を行うために使用します。
  - [DateTime](https://pypi.org/project/DateTime/)
- **Pillow (PIL)**: 画像の処理を行うために使用します。
  - [Pillow](https://pypi.org/project/pillow/)
- **fpdf**: 画像をPDFに変換するために使用します。
...
1. **Kindle2PDF.pyの設定**
   - `Kindle2PDF.py` ファイルを編集し、以下の変数に先ほど取得した座標を入力します。
...
3. **実行する（Run the Script）**
   ...
   - その後、指定した座標内でスクリーンショットが自動で行われるため、スクリプトが完了するまで **他の操作を行わないでください**。PCがスリープしたりしてもいけないので、予めスリープ設定をオフにしておくことをお勧めします。
